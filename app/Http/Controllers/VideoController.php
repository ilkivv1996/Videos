...
class VideoController extends Controller
{
    ...
    public function index(Video $vd, Apikey $pk, Menu $menu){ // главная страница
        ...
        $search = ["xiaomi", "как сделать", "бег", "отдых на море", "как отдыхать в палатке", "замена топливного фильтра", "ремонт платы", "клипы", "cs:go", "far cry 5", "сборка пк"];
        ...
        $this->data['videos'] = $vd->getRandomMain(10);
    ...
    public function save_video($title, $video, $tags, $img){ // сохраняем полученные видео в бд
        ...
        $vd->path_url = '/videos/' . $this->translit($tags) . '/' .$this->translit($title);
        $vd->video = $video;
        $vd->tags = $tags;
        $vd->img = $img;
        $vd->save();
